...
for time=(0:N-1)*timestep
  ...
  %事件驱动
  if norm(x-Xetm,2)<delta*norm(x,2)
    thetahat=thetahat+(APHI(Xetm)*K*Xetm).*timestep;
  else
...
plot(Ttime,Ustore)
   ...
   save InvertIX Xstore
   save InvertITheta Thetastore
   save InvertIFlag Flagstore
   save InvertIEventsNumber EventsNumber
   save InvertIUstore Ustore
