...
for time=(0:N-1)*timestep
  ...
  %状态更新
  x=[x0(2), theta'*phi(x0(1))+u+d*sin(time)]'.*timestep+x0;%+0.1*randn
  %事件驱动
  if norm(x-[setpoint,0]'-Xetm,2)<delta*norm(x-[setpoint,0]',2)
      thetahat=thetahat+(K'*Xetm*phi(Xetm(1)+setpoint)).*timestep;
  else
...
plot(Ttime,Ustore)
 ...
 save TTT Ttime
 save TZstore1 Zstore
 save TXstore1 Xstore
 save TThetastore1 Thetastore
 save TEventsNumber1 EventsNumber
 save TUstore1 Ustore
 save TrueTheta TrueTheta
